...
ParsedLine Parser::parseLine(TokenStream& tokens,
  ...
  if (firstToken && firstToken->type == TokenType::NUMBER) {
    ...
    // Extract statement text without line number
    // Find where the line number ends in the original string
    std::string lineNumStr = std::to_string(lineNum);
    size_t pos = originLine.find(lineNumStr);
    if (pos != std::string::npos) {
      // Skip the line number and any following spaces
      pos += lineNumStr.length();
      while (pos < originLine.length() && std::isspace(originLine[pos])) {
        pos++;
      }
      stmtText = originLine.substr(pos);
    }
  }
